...
const Vacheron = () => {
  ...
  const [mainImage, setMainImage] = useState(
    `${process.env.PUBLIC_URL}/img/Vacheron1.jpeg`
  );
  const additionalImages = [
    `${process.env.PUBLIC_URL}/img/Vacheron1.jpeg`,
    `${process.env.PUBLIC_URL}/img/Vacheron2.jpeg`,
    `${process.env.PUBLIC_URL}/img/Vacheron3.jpeg`,
  ];
  ...
  return (
    <article className='py-8 px-4 lg:px-32 gap-4 flex flex-col lg:flex-row min-h-screen pt-20 overflow-hidden'>
        ...
        <div className='flex justify-center space-x-4 mt-4'>
          {additionalImages.map((image, index) => (
            <img
              ...
              className='w-20 md:w-24 lg:w-32 mt-6 mr-6 cursor-pointer'
      ...
      <div className='flex-1 m-4 p-4 lg:pb-96 font-libre text-indigo-600 w-full'>
        <h1 className='text-2xl pb-4 lg:text-3xl'>₦120,000,000.00</h1>
        ...
        <br />
        <button className='bg-indigo-800 text-slate-300 w-80 md:w-96 h-10 mb-4 hover:bg-slate-500 hover:text-indigo-600'>
          BUY NOW
        </button>
